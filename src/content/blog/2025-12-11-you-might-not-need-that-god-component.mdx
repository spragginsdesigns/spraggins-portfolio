...
Here's what I learned breaking it apart - and why you might want to do the same.
...
│       ├── DashboardHeader.tsx    // Header UI + logic
...
- ✅ "DashboardHeader renders the header with user info and navigation"
- ✅ "useDashboardData fetches and caches dashboard statistics"
- 🔴 "Dashboard handles the header, stats, activity feed, filters, user preferences, notifications, and also some shared state that other components need"
...
**Why it matters:** The "avoid" version renders twice - once when `items` changes, then again when the effect runs. The "good" version renders once.

### Pattern 2: Responding to Events

```tsx
// 🔴 Avoid: Effect to respond to form submission
...
- Component appeared → useEffect (maybe)

### Pattern 3: Fetching on Mount (The Only Valid One)

```tsx
// ✅ Actually good: Data fetching on mount
useEffect(() => {
  fetchDashboardData().then(setData);
}, []);
```

This is the one place useEffect makes sense - synchronizing with an external system (your API) when the component mounts. Even then, consider using a data fetching library that handles caching, deduplication, and loading states for you.
...
I use AI assistance every single day now. My role has shifted from "person who writes code" to "person who designs systems and reviews AI output." Organizing for AI readability isn't optional anymore - it's a competitive advantage.
...
3. **Treat AI readability as a feature.** It's not just about you anymore. Your codebase has another reader now - make it easy for them too.
